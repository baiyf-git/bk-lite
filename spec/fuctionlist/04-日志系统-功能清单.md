...
**编制依据：** 日志系统 PRD（2026-06-22 重构版，拆分为集成/日志搜索/日志分析/事件 四份）与 `server/apps/log` 源代码核对；增量基线 77bb6c6 → HEAD
...
| 关键字告警 | 关键字告警支持展示字段配置；支持按分组字段（group_by）拆分，每个分组独立成一条告警并保留各自样本；分组告警名称通过 `${field}` 占位符渲染各组字段值 | 分组样本查询并发执行（默认最多 10 线程，可通过 `LOG_GROUPED_ALERT_MAX_WORKERS` 调整）；无 group_by 时退化为单条聚合告警 | GA |
...
| 通知可靠性 | 通知发送支持内联重试（默认最多 3 次，线性退避）；未成功通知的事件由周期补偿任务自动重投（单事件最多 5 次，补偿窗口默认 24 小时），语义为 at-least-once | 补偿 Celery Beat 任务每 5 分钟回扫一次；内联重试次数与补偿次数/窗口可通过 `LOG_NOTICE_SEND_MAX_ATTEMPTS`/`LOG_NOTICE_COMPENSATE_MAX_RETRY`/`LOG_NOTICE_COMPENSATE_WINDOW_SECONDS` 等环境变量调整；Event 模型新增 `notified`/`notice_retry_count` 字段支撑幂等去重 | GA |
| 通知去重与级别变化重通知 | 同一活跃告警成功通知后不重复发送；告警级别升/降时重置通知状态并触发一次新通知 | — | GA |
...
| 原始日志快照 | 告警生命周期内累积存储事件原始数据快照 | 快照存于对象存储（S3/MinIO），自动压缩；单告警快照保留最新 500 条，超限丢弃最旧（上限可通过环境变量调整） | GA |
| 告警关闭 | 人工关闭告警并记录关闭操作人 | 列表与详情可查看关闭操作人；关闭前执行 `Operate` 权限校验，无权限返回 403 | GA |
...
| Kubernetes 接入 | 支持云区域选择、安装命令生成与采集状态检查 | 集群名称仅校验非空，不限制字符格式；Infra API 回调启用 TLS 证书校验 | GA |
| 采集插件 | 内置采集插件 | Filebeat、Auditbeat、Packetbeat、Winlogbeat、Snmptrapd、Vector | GA |
| Packetbeat 网络流量采集配置 | 网络流量采集支持分别开关 HTTP 与 TCP/UDP 抓取、配置 HTTP 监听端口（1–65535 校验），并按节点操作系统规范化监听网卡（Linux 归一、Windows 用索引）；Packetbeat http 子类型不单独在采集类型列表展示 | — | GA |
...
| 日志分组管理 | 日志分组的创建、查询、更新、删除 | 规则支持 AND 与 OR 组合；创建时通过 `scope=log_group_create` 模式查询字段候选，范围精确限定为当前用户可管理组织下各采集实例上报字段（按 `instance_id` OR 拼装查询，不经过日志分组权限路径） | GA |
| 默认日志分组 | 系统提供默认日志分组用于全量日志场景 | 选择默认分组时按全量日志口径查询；默认分组与其他分组同时选择时以默认分组优先，其他分组条件不参与查询拼装 | GA |
| 节点信息查询 | 在接入场景查询节点信息，辅助实例接入与专用接入指引展示 | — | GA |

## 三、能力边界与约束

本模块不覆盖非日志数据域（指标、链路、资产等）的统一分析与治理，不提供第三方日志平台的深度双向编排能力。日志分析页仅提供内置仪表盘查看，不承诺用户自定义仪表盘的保存与编排。策略编排能力以关键字告警与聚合告警两类为限，不承诺通用规则引擎。告警处置状态以活跃（`new`）与关闭（`closed`）为主，关闭通过人工操作完成并记录操作人；关闭前执行策略级 `Operate` 权限校验，无权限返回 403。搜索条件按当前组织隔离，不随"包含子组织"设置扩展。日志查询设安全延迟（默认 60 秒），数据补偿单次上限 10 个周期、最大 24 小时。日志模块进入后默认跳转到首个可用菜单项。所有关键查询与管理操作受组织权限与实例权限约束，非授权范围内数据不可见、不可改。通知可靠性以 at-least-once 为目标，补偿任务由 Celery Beat 每 5 分钟触发一次回扫；不保证 exactly-once，接收方应具备幂等处理能力。日志分组创建时字段候选查询路径与检索路径分离，使用 `scope=log_group_create` 参数触发，按用户可管理组织范围内的采集实例 ID 直接构建过滤条件。

## 四、平台协同

日志系统从节点管理获取节点信息并保持采集实例与节点侧配置一致；通知渠道与可用用户信息来源于系统管理统一配置；组织隔离与权限控制依赖系统管理 RBAC；日志策略产生的告警可作为标准化事件源向告警中心 `/alarm` 输送（告警源类型含"日志"），由告警中心完成跨源聚合与处置协同；原始日志快照存于对象存储（S3/MinIO）。NATS `log_search`/`log_hits` 接口在执行前应用日志分组权限过滤（通过 `user_info` 参数解析），超级管理员不受分组限制，普通用户无可访问分组时返回空集。
...
> 证据来源（增量修订 77bb6c6 → HEAD）：`models/policy.py:100-101`（Event.notified/notice_retry_count 新字段）；`migrations/0016_event_notified_event_notice_retry_count.py`（字段迁移）；`config.py`（Celery Beat 每 5 分钟补偿调度）；`constants/alert_policy.py:13-25`（通知阈值常量，`LOG_NOTICE_*` 环境变量）；`tasks/policy.py:compensate_log_notice_task`（补偿任务）；`tasks/services/policy_scan.py:_keyword_grouped_alert_detection,_fetch_group_sample,_render_alert_name,send_notice,notice,_update_alert_snapshot`（分组告警/并发采样/名称渲染/内联重试/去重/快照上限）；`serializers/policy.py:get_alert_name`（告警名使用 content 优先）；`views/policy.py:AlertViewSet.closed,_authorize_alert_operate`（关闭前 Operate 权限校验）；`services/k8s_collect.py:validate_cluster_name,get_webhook_tls_verify`（集群名不限格式/TLS 校验恢复）；`services/access_scope.py:get_manageable_organization_ids,resolved_group_objects`（创建分组可管理组织 ID 接口）；`views/collect_config.py:_get_log_group_create_attrs,_is_log_group_create_scope`（scope=log_group_create 字段发现）；`nats/log.py:_apply_log_group_scope,_resolve_log_group_scope`（NATS 搜索应用分组权限）；`views/search.py:SearchConditionViewSet.get_queryset`（.only()/.iterator() 优化）　|　同步基线：HEAD（dc733f5b8）　|　【已实现】

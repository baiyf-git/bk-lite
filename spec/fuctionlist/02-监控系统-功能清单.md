...
**编制依据：** 监控系统 PRD v1.8（2026-05-28）与 `server/apps/monitor`、`web/src/app/monitor` 源代码核对；2026-06-19 增量更新：NetFlow/sFlow 8 个流量插件指标扩展、Website 新增 Probe Result 指标
...
_NetFlow flow analysis template for firewall devices. It maps exporter traffic to monitored assets, normalizes sampling rate, and provides overview, interface, protocol, port, endpoint, and conversation metrics._

| 指标分组 | 指标名 | 显示名 | 单位 | 中文含义 |
|---|---|---|---|---|
| Traffic Overview | `device_flow_bytes_rate` | Device Flow Traffic Rate | byteps | 设备 NetFlow 归一化后的总流量速率（入向+出向） |
| Traffic Overview | `device_flow_packets_rate` | Device Flow Packet Rate | pps | 设备 NetFlow 归一化后的总报文速率（入向+出向） |
| Traffic Overview | `device_flow_in_bytes_rate` | Device Inbound Flow Traffic Rate | byteps | 设备 NetFlow 归一化后的入向流量速率 |
| Traffic Overview | `device_flow_out_bytes_rate` | Device Outbound Flow Traffic Rate | byteps | 设备 NetFlow 归一化后的出向流量速率 |
| Traffic Overview | `device_flow_in_packets_rate` | Device Inbound Flow Packet Rate | pps | 设备 NetFlow 归一化后的入向报文速率 |
| Traffic Overview | `device_flow_out_packets_rate` | Device Outbound Flow Packet Rate | pps | 设备 NetFlow 归一化后的出向报文速率 |
| Traffic Overview | `device_flow_avg_packet_size` | Device Flow Average Packet Size | bytes | 基于归一化字节数与报文数计算的平均报文大小 |
| Interface Traffic | `device_flow_interface_bytes_rate` | Interface Flow Traffic Rate | byteps | 按接口与方向分组的归一化 NetFlow 流量速率 |
| Interface Traffic | `device_flow_interface_packets_rate` | Interface Flow Packet Rate | pps | 按接口与方向分组的归一化 NetFlow 报文速率 |
| Interface Traffic | `device_flow_top_interfaces_by_bytes` | Top Interfaces by Flow Traffic Rate | byteps | 按归一化流量速率排序的 Top 接口（取前 10） |
| Protocol | `device_flow_protocol_bytes_rate` | Protocol Flow Traffic Rate | byteps | 按协议分组的归一化 NetFlow 流量速率 |
| Protocol | `device_flow_protocol_packets_rate` | Protocol Flow Packet Rate | pps | 按协议分组的归一化 NetFlow 报文速率 |
| Application Port | `device_flow_dst_port_bytes_rate` | Destination Port Flow Traffic Rate | byteps | 按目标端口分组的归一化 NetFlow 流量速率 |
| Application Port | `device_flow_dst_port_packets_rate` | Destination Port Flow Packet Rate | pps | 按目标端口分组的归一化 NetFlow 报文速率 |
| Application Port | `device_flow_src_port_bytes_rate` | Source Port Flow Traffic Rate | byteps | 按源端口分组的归一化 NetFlow 流量速率 |
| Endpoint | `device_flow_top_src_bytes_rate` | Top Source Flow Traffic Rate | byteps | 按归一化流量速率排序的 Top 源地址（取前 10） |
| Endpoint | `device_flow_top_dst_bytes_rate` | Top Destination Flow Traffic Rate | byteps | 按归一化流量速率排序的 Top 目标地址（取前 10） |
| Endpoint | `device_flow_top_src_packets_rate` | Top Source Flow Packet Rate | pps | 按归一化报文速率排序的 Top 源地址（取前 10） |
| Endpoint | `device_flow_top_dst_packets_rate` | Top Destination Flow Packet Rate | pps | 按归一化报文速率排序的 Top 目标地址（取前 10） |
| Conversation | `device_flow_top_conversation_bytes_rate` | Top Conversation Flow Traffic Rate | byteps | 按归一化流量速率排序的 Top 会话（src/dst/协议/目标端口，取前 10） |
| Sampling | `device_flow_effective_sampling_rate` | Device Flow Effective Sampling Rate | none | NetFlow 流量归一化使用的有效采样率 |

### Loadbalance Flow NetFlow · Loadbalance（21 指标）
采集引擎：Telegraf、对象类别：Network Device  
_NetFlow flow analysis template for loadbalance devices. It maps exporter traffic to monitored assets, normalizes sampling rate, and provides overview, interface, protocol, port, endpoint, and conversation metrics._
...
采集引擎：Telegraf、对象类别：Network Device  
_NetFlow flow analysis template for router devices. It maps exporter traffic to monitored assets, normalizes sampling rate, and provides overview, interface, protocol, port, endpoint, and conversation metrics._
...
采集引擎：Telegraf、对象类别：Network Device  
_NetFlow flow analysis template for switch devices. It maps exporter traffic to monitored assets, normalizes sampling rate, and provides overview, interface, protocol, port, endpoint, and conversation metrics._
...
采集引擎：Telegraf、对象类别：Network Device  
_sFlow flow analysis template for firewall devices. It maps exporter traffic to monitored assets, normalizes sampling rate, and provides overview, interface, protocol, port, endpoint, and conversation metrics._

| 指标分组 | 指标名 | 显示名 | 单位 | 中文含义 |
|---|---|---|---|---|
| Traffic Overview | `device_flow_bytes_rate` | Device Flow Traffic Rate | byteps | 设备 sFlow 归一化后的总流量速率（入向+出向） |
| Traffic Overview | `device_flow_packets_rate` | Device Flow Packet Rate | pps | 设备 sFlow 归一化后的总报文速率（入向+出向） |
| Traffic Overview | `device_flow_in_bytes_rate` | Device Inbound Flow Traffic Rate | byteps | 设备 sFlow 归一化后的入向流量速率 |
| Traffic Overview | `device_flow_out_bytes_rate` | Device Outbound Flow Traffic Rate | byteps | 设备 sFlow 归一化后的出向流量速率 |
| Traffic Overview | `device_flow_in_packets_rate` | Device Inbound Flow Packet Rate | pps | 设备 sFlow 归一化后的入向报文速率 |
| Traffic Overview | `device_flow_out_packets_rate` | Device Outbound Flow Packet Rate | pps | 设备 sFlow 归一化后的出向报文速率 |
| Traffic Overview | `device_flow_avg_packet_size` | Device Flow Average Packet Size | bytes | 基于归一化字节数与报文数计算的平均报文大小 |
| Interface Traffic | `device_flow_interface_bytes_rate` | Interface Flow Traffic Rate | byteps | 按接口与方向分组的归一化 sFlow 流量速率 |
| Interface Traffic | `device_flow_interface_packets_rate` | Interface Flow Packet Rate | pps | 按接口与方向分组的归一化 sFlow 报文速率 |
| Interface Traffic | `device_flow_top_interfaces_by_bytes` | Top Interfaces by Flow Traffic Rate | byteps | 按归一化流量速率排序的 Top 接口（取前 10） |
| Protocol | `device_flow_protocol_bytes_rate` | Protocol Flow Traffic Rate | byteps | 按协议分组的归一化 sFlow 流量速率 |
| Protocol | `device_flow_protocol_packets_rate` | Protocol Flow Packet Rate | pps | 按协议分组的归一化 sFlow 报文速率 |
| Application Port | `device_flow_dst_port_bytes_rate` | Destination Port Flow Traffic Rate | byteps | 按目标端口分组的归一化 sFlow 流量速率 |
| Application Port | `device_flow_dst_port_packets_rate` | Destination Port Flow Packet Rate | pps | 按目标端口分组的归一化 sFlow 报文速率 |
| Application Port | `device_flow_src_port_bytes_rate` | Source Port Flow Traffic Rate | byteps | 按源端口分组的归一化 sFlow 流量速率 |
| Endpoint | `device_flow_top_src_bytes_rate` | Top Source Flow Traffic Rate | byteps | 按归一化流量速率排序的 Top 源地址（取前 10） |
| Endpoint | `device_flow_top_dst_bytes_rate` | Top Destination Flow Traffic Rate | byteps | 按归一化流量速率排序的 Top 目标地址（取前 10） |
| Endpoint | `device_flow_top_src_packets_rate` | Top Source Flow Packet Rate | pps | 按归一化报文速率排序的 Top 源地址（取前 10） |
| Endpoint | `device_flow_top_dst_packets_rate` | Top Destination Flow Packet Rate | pps | 按归一化报文速率排序的 Top 目标地址（取前 10） |
| Conversation | `device_flow_top_conversation_bytes_rate` | Top Conversation Flow Traffic Rate | byteps | 按归一化流量速率排序的 Top 会话（src/dst/协议/目标端口，取前 10） |
| Sampling | `device_flow_effective_sampling_rate` | Device Flow Effective Sampling Rate | none | sFlow 流量归一化使用的有效采样率 |

### Loadbalance Flow sFlow · Loadbalance（21 指标）
采集引擎：Telegraf、对象类别：Network Device  
_sFlow flow analysis template for loadbalance devices. It maps exporter traffic to monitored assets, normalizes sampling rate, and provides overview, interface, protocol, port, endpoint, and conversation metrics._
...
采集引擎：Telegraf、对象类别：Network Device  
_sFlow flow analysis template for router devices. It maps exporter traffic to monitored assets, normalizes sampling rate, and provides overview, interface, protocol, port, endpoint, and conversation metrics._
...
采集引擎：Telegraf、对象类别：Network Device  
_sFlow flow analysis template for switch devices. It maps exporter traffic to monitored assets, normalizes sampling rate, and provides overview, interface, protocol, port, endpoint, and conversation metrics._
...
| 指标分组 | 指标名 | 显示名 | 单位 | 中文含义 |
|---|---|---|---|---|
| HTTP | `http_response_result_code` | Probe Result | Enum | 探测结果码枚举（0 成功/1 响应内容不匹配/2 响应体读取失败/3 连接失败/4 超时/5 DNS错误/6 响应状态码不匹配）；来源：server/apps/monitor/support-files/plugins/Telegraf/web/web/metrics.json:1-30 |

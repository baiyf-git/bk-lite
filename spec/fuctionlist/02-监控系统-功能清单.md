...
**编制依据：** 监控系统 PRD（2026-06-22）与 `server/apps/monitor`、`web/src/app/monitor` 源代码核对；本次相对 V1.0（2026-06-02 基线）的增量更新：新增 HAProxy / Windows WMI 插件、扩充 Host Remote / PostgreSQL 指标、完善专业仪表板 / 有效插件筛选 / 展示列配置 / 批量应用策略模板 / 流量接入向导等功能条目，并更新各对象指标计数与总量
...
平台内置约 71 个采集插件（含 26 个品牌级 SNMP 插件、8 个 NetFlow/sFlow 流量插件），展开为约 40 个监控对象（含复合对象的子对象），合计预置约 **1203 项指标**，开箱即用、无需手动定义。下表按采集引擎与对象类别列出，括号内为该对象预置指标数（基于内置 `metrics.json` 统计，口径：各对象 `metrics` 数组项数之和，品牌 SNMP 插件指标独立计入其对应对象）。
...
交换机、防火墙、路由器、负载均衡四类网络设备对象另按品牌提供 26 个专用 SNMP 插件，采集各厂商私有 OID 设备健康指标：

| 品牌插件 | 映射对象 | 专属指标数 |
|---|---|---|
| Switch Alcatel-Lucent / Aruba / Brocade / Cisco / Dell Force10 / D-Link / Extreme / HP ProCurve / Huawei / Juniper / MikroTik / Netgear / QTech / TP-Link / Zyxel SNMP | Switch 交换机 | 4–9 / 品牌 |
| Firewall Check Point / Fortinet / OPNsense / Palo Alto / pfSense / SonicWall / Stormshield / WatchGuard SNMP | Firewall 防火墙 | 18–24 / 品牌 |
| Router Huawei AR / Juniper MX SNMP | Router 路由器 | 18 / 品牌 |
| Loadbalance F5 SNMP | Loadbalance 负载均衡 | 21 |

> 证据来源：`server/apps/monitor/support-files/plugins/Telegraf/snmp_*/*/metrics.json`（26 个品牌插件目录，plugin 字段如 "Switch Huawei SNMP"）　|　同步基线：0fbb99c25　|　【已实现】
...
> 本节逐项列出各内置监控对象的预置指标，源自各采集插件 `metrics.json`（与代码一致）。共约 71 个插件（含 26 个品牌级 SNMP 插件、8 个 NetFlow/sFlow 流量插件、HAProxy、Windows WMI 等新插件）/ 约 40 个监控对象、约 1203 项指标；中文含义取自人工校对口径，缺失处以英文显示名为准。每项指标均带多维标签（如 `instance_id`、设备/分区/接口等维度），实际可观测维度多于条目数。品牌级 SNMP 插件指标详见 §5.2 品牌插件表格，本节不逐品牌展开。

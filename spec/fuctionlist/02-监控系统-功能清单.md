...
**编制依据：** 监控系统 PRD（2026-06-23）与 `server/apps/monitor`、`web/src/app/monitor` 源代码核对；本版增量基线 0fbb99c2（2026-06-23 核对）；本次相对 V1.1 的增量更新：扩充 SNMP 网络设备厂商插件至 61 个（Switch×37 / Router×7 / Firewall×12 / Loadbalance×5）、新增策略配置预览功能条目、新增展示列名国际化功能条目、更新内置对象与插件总量
...
| 专业仪表板 | 按监控对象提供预置首屏观测视图，覆盖操作系统、网络、数据库、中间件等分组；仪表板支持实例切换、时间范围调整、自动/手动刷新与「仪表板/指标」模式切换 | 当前内置对象：Host、Website、Ping、MySQL、Redis、MongoDB、MSSQL、PostgreSQL、Elasticsearch、Nginx、Docker、ActiveMQ、Apache、Consul、RabbitMQ、Tomcat、Zookeeper 及网络设备 Switch/Router/Firewall/Loadbalance；侧边栏按分组折叠展开并支持搜索高亮；跨对象别名（如 website/web、postgres/postgresql）统一映射 | GA |
| 有效插件筛选 | 视图实例列表仅展示实际已上报数据的采集插件，过滤无数据的空插件 | 通过 `GET /effective_plugins` 接口判断；需传入 `instance_id` 且用户有该实例权限 | GA |
| 实例列表展示列配置 | 为每个监控对象配置在实例列表中默认展示的关键指标列（display_fields），支持管理员保存自定义列顺序与指标选择 | 由 `MonitorObject.display_fields` 存储（`JSONField`）；初始值由 `metrics.json` 中 `display_fields` 字段种子注入；用户保存后 `display_fields_customized=True`，不再被初始化覆盖 | GA |
...
| 策略配置预览 | 策略配置页可对已填写的查询条件、检测周期、聚合算法、分组维度和单位换算进行服务端实时预览；返回生成的查询语句、近期数据点、最终展示单位；单位换算不可执行时返回提示；无需保存策略即可触发 | POST `/api/v1/monitor/monitor_policy/preview/`；服务端执行，即时返回；不持久化任何数据（证据：`server/apps/monitor/views/monitor_policy.py:466-468`） | GA |
...
| 批量应用策略模板 | 一次提交多个模板配置，批量为同一监控对象创建策略 | 通过 `POST bulk_create_from_templates` 接口；传入 `monitor_object_id` 与模板列表，后端自动补全来源、组织、缺失字段并批量入库 | GA |
...
| 流量监控接入向导 | 为 NetFlow/sFlow 网络流量监控提供三步接入向导：绑定资产 → 接入指引 → 完成 | 支持交换机、路由器、防火墙、负载均衡四类对象；绑定资产步骤可新建或复用已有资产，登记云区域、设备 IP、组织与兜底采样率；同一云区域 + IP 唯一，重复绑定合并协议而非新建；接入指引展示监听地址（NetFlow/sFlow 按协议区分端口）与设备侧导出说明，可检测是否已收到实际流量数据 | GA |
...
| 内置对象覆盖 | 平台内置监控对象覆盖多类资源 | 已随包**预置标准指标集**（开箱即用）的对象见"五、支持的监控对象与指标范围"，约 96 个厂商级采集插件（含 61 个 SNMP 网络设备厂商插件、8 个 NetFlow/sFlow 流量插件、HAProxy、Windows WMI 等）/ 约 40 个对象 / 约 1203 项指标（主机/DB/中间件等）。注：对象类型列表中另有 **ClickHouse、SNMP Trap** 等条目，其对象类型已内置但**未随包预置标准指标集，需结合自建模板（API/PULL/SNMP）配置后使用**，不计入开箱预置范围 | GA（ClickHouse / SNMP Trap 需模板扩展） |
| 展示列名国际化 | 监控对象实例列表中各列的列名一律显示为该列绑定指标在当前账号语言下的译名，随账号语言（中/英）自动切换；在列配置弹窗中添加、删除或排序列后，列名仍跟随语言切换，不回退为英文原始标签 | 无绑定指标或无对应译名时回退显示原列名（证据：`server/apps/monitor/views/monitor_object.py:40-61`） | GA |

## 三、能力边界与约束

监控系统范围严格限定 `/monitor`，不含独立告警平台 `/alarm` 的事故、分派、屏蔽、系统设置等能力；移动端当前仅保留工作台"监控告警"入口，不建设完整移动端监控功能。蜂巢视图仅 Pod / Node 支持，其余对象为列表视图。维度表仅在单列布局下展示。集成详情页默认仅包含 Configure 与 Metric，Collect 仅对 SNMP 模板开放；自建模板仅覆盖 API、PULL、SNMP 三类。告警关闭等敏感操作按权限点控制，命名查询保存须绑定组织。`info` 级别事件仅用于记录恢复判断过程，不参与告警升级、不生成新的活跃告警。数据补偿单次上限 30 个周期、超过 24 小时不再补偿。本模块不新增采集引擎，采集依赖既有插件与节点通道。策略配置预览接口在服务端执行、即时返回，不持久化任何数据。监控对象实例列表列名跟随账号语言自动切换，编辑列配置后不回退英文。网络设备 SNMP 插件当前支持 Switch / Router / Firewall / Loadbalance 四类对象，共 61 个厂商插件，不支持范围外对象类型或未列出品牌的开箱预置采集（可通过自建 SNMP 模板扩展）。流量监控接入向导中，同一云区域 + IP 的流量资产唯一，重复绑定合并已启用协议而非新建实例；接入向导仅支持 NetFlow/sFlow 协议流量资产，不适用于其他监控对象。Windows WMI 插件以 HTTP collect 类型接入，与 Host Remote（SSH/WinRM via Ansible Executor）为独立插件，共同映射到 Host 对象。实例列表展示列（display_fields）用户保存后不再被系统初始化覆盖。
...
平台内置约 96 个厂商级采集插件（含 61 个品牌级 SNMP 插件、8 个 NetFlow/sFlow 流量插件及主机/DB/中间件/容器等约 27 个通用插件），展开为约 40 个监控对象（含复合对象的子对象），合计预置约 **1203 项指标**，开箱即用、无需手动定义。下表按采集引擎与对象类别列出，括号内为该对象预置指标数（基于内置 `metrics.json` 统计，口径：各对象 `metrics` 数组项数之和，品牌 SNMP 插件指标独立计入其对应对象）。
...
| 远程主机（无 Agent） | Remote Host（52，通过 Ansible Executor SSH/WinRM 采集） | Telegraf (HTTP) |
| Windows 主机（WMI） | Windows WMI Host（28，通过 HTTP 采集器对接 WMI 接口） | Telegraf (HTTP) |
...
| 监控对象（指标数） | 采集引擎 |
|---|---|
| Switch 交换机（14）、Router 路由器（14）、Firewall 防火墙（14）、Loadbalance 负载均衡（14） | Telegraf (SNMP 通用) |

交换机、防火墙、路由器、负载均衡四类网络设备对象另按品牌提供 **61 个专用 SNMP 插件**，采集各厂商私有 OID 设备健康指标（指标数因厂商而异，约 3–24 项/插件）：

| 对象类型 | 厂商品牌数 | 支持厂商品牌（全量） | 采集引擎 |
|---|---|---|---|
| Switch 交换机 | 37 | 锐捷（Ruijie）、华三（H3C）、华为（Huawei）、思科（Cisco）、Arista、Juniper、戴尔 OS10（Dell OS10）、联想 CNOS（Lenovo CNOS）、Mellanox、Allied Telesis、Ubiquiti、3Com、ZTE、Datacom、DCN、Eltex、Fiberhome、FortiSwitch、FS、Hirschmann、Netonix、Alcatel-Lucent（AOS）、OmniSwitch（ALE）、Parks、SNR、Yamaha、Aruba、Brocade、D-Link、Extreme、Netgear、QTech、TP-Link、Zyxel、DellForce、HP HPN、MikroTik | Telegraf (SNMP) |
| Router 路由器 | 7 | 华为 AR（Huawei AR）、Juniper MX、Adtran、Draytek、Lancom、NEC、Vyatta | Telegraf (SNMP) |
| Firewall 防火墙 | 12 | CheckPoint、Fortinet、Palo Alto、SonicWall、Sophos、Hillstone（山石）、ScreenOS（Juniper）、Forcepoint、opnSense、pfSense、Stormshield、WatchGuard | Telegraf (SNMP) |
| Loadbalance 负载均衡 | 5 | F5、A10、Alteon（Radware）、FortiADC、NetScaler（Citrix ADC） | Telegraf (SNMP) |

> 本节所列 61 个 SNMP 厂商插件均为 GA（随包预置、开箱即用）。每个插件均包含 SNMP 采集配置模板、预置指标定义及告警策略模板；指标数因厂商而异（约 3–24 项/插件），涵盖接口流量、CPU、内存、系统运行时长、设备健康状态、会话等维度。对应 [[集成.md § 3.1.1]]。

> 证据来源：`server/apps/monitor/support-files/plugins/Telegraf/snmp_*/*/metrics.json`（61 个品牌插件目录，plugin 字段如 "Switch Huawei SNMP"）　|　同步基线：0fbb99c2　|　【已实现】

### 5.3 数据库

| 监控对象（指标数） | 采集引擎 |
|---|---|
| MySQL（70）、PostgreSQL（23）、Redis（16）、MongoDB（29）、MSSQL（26）、Elasticsearch（14）、InfluxDB（9） | Telegraf |
...
| 监控对象（指标数） | 采集引擎 |
|---|---|
| Nginx（7）、Apache（19）、Tomcat（20）、Zookeeper（20）、RabbitMQ（20）、ActiveMQ（4）、Consul（6）、HAProxy（33） | Telegraf |
...
| 监控对象（指标数） | 采集引擎 |
|---|---|
| Website 网站拨测（5）、Ping 连通性（6） | Telegraf |

### 5.7 网络流量（NetFlow/sFlow）

面向交换机、路由器、防火墙、负载均衡四类网络设备，每类设备分别提供 NetFlow 与 sFlow 两个插件，共 **8 个流量插件 × 21 项 `device_flow_*` 指标**，覆盖设备总流量、方向分流、接口 Top-N、协议分布、端口分布与会话 Top-N 等维度。指标详见 §6 Firewall/Switch/Router/Loadbalance Flow NetFlow/sFlow。

| 流量协议 | 监控对象（指标数） | 采集引擎 |
|---|---|---|
| NetFlow | Switch（21）、Router（21）、Firewall（21）、Loadbalance（21） | Telegraf (NetFlow 接收器) |
| sFlow | Switch（21）、Router（21）、Firewall（21）、Loadbalance（21） | Telegraf (sFlow 接收器) |

> 证据来源：`server/apps/monitor/support-files/plugins/Telegraf/netflow/*/metrics.json` 与 `sflow/*/metrics.json`（每插件 21 项，feat(monitor): enrich flow traffic metrics 2026-06-17）　|　同步基线：2026-06-22　|　【已实现】

> Website 拨测包含「探测结果（Probe Result，`http_response_result_code`）」与「节点成功率（`http_node_success_rate`）」两项关键指标：前者枚举含成功/内容不匹配/响应体读取失败/连接失败/超时/DNS 错误/状态码不匹配，带 `agent_id` 维度按探测节点区分，置为首位 display_field；后者统计各节点最近 5 分钟拨测成功率。证据：`server/apps/monitor/support-files/plugins/Telegraf/web/web/metrics.json`（5 项，feat(monitor): add website probe result metric 2026-06-17）　|　同步基线：2026-06-22　|　【已实现】
...
> 本节逐项列出各内置监控对象的预置指标，源自各采集插件 `metrics.json`（与代码一致）。共约 96 个厂商级插件（含 61 个品牌级 SNMP 插件、8 个 NetFlow/sFlow 流量插件、HAProxy、Windows WMI 等）/ 约 40 个监控对象、约 1203 项指标；中文含义取自人工校对口径，缺失处以英文显示名为准。每项指标均带多维标签（如 `instance_id`、设备/分区/接口等维度），实际可观测维度多于条目数。品牌级 SNMP 插件指标详见 §5.2 品牌插件表格，本节不逐品牌展开。
...
> 证据来源：`server/apps/monitor/support-files/plugins/Telegraf/database/postgres/metrics.json`（23 项，fix(monitor): add 3 missing postgres metrics 2026-06-07）　|　V1.0 基线为 20 项，新增 `postgresql_blk_write_time`、`postgresql_active_time`、`postgresql_sessions_killed` 共 3 项
...
| WriteActivity | `postgresql_blk_write_time` | Disk Block Write Time Rate | ms | 后端向数据文件写入数据块所花费的时间速率，反映磁盘写入压力。 |
| Activity | `postgresql_active_time` | SQL Active Execution Time Rate | ms | 处于活跃状态的 SQL 语句累计执行时间速率，反映数据库整体繁忙度。 |
| Activity | `postgresql_sessions_killed` | Abnormally Terminated Sessions Rate | cps | 因超时或错误被强制终止的会话速率，反映异常连接中断频率。 |
...
### Host Remote · Host（52 指标）
采集引擎：Telegraf、对象类别：OS  
_Remote host metrics collection via SSH/WinRM through Ansible Executor. Supports CPU, memory, disk, network, disk I/O and process metrics without agent installation on target hosts._

> 证据来源：`server/apps/monitor/support-files/plugins/Telegraf/http/host/metrics.json`（52 项，feat: align host network metrics and expand windows wmi metrics 2026-06-09）　|　V1.0 基线为 18 项，本次扩充 +34 项
...
| CPU | `cpu_usage_user_total` | CPU User Usage | % | CPU执行用户态应用的时间占比 |
| CPU | `cpu_usage_system_total` | CPU System Usage | % | CPU执行内核态任务的时间占比 |
| CPU | `cpu_usage_iowait_total` | CPU IOWait Usage | % | CPU等待I/O完成的时间占比 |
| CPU | `cpu_usage_irq_total` | CPU IRQ Usage | % | CPU处理硬件中断的时间占比 |
| CPU | `cpu_usage_steal_total` | CPU Steal Usage | % | 虚拟化环境中被宿主机抢占的CPU时间占比 |
| System | `system_load1` | Load 1m | — | 系统近1分钟平均负载 |
| System | `system_load5` | Load 5m | — | 系统近5分钟平均负载 |
| System | `system_load15` | Load 15m | — | 系统近15分钟平均负载 |
| System | `system_uptime` | System Uptime | s | 系统持续运行时长 |
...
| Memory | `mem_cached` | Cached Memory | bytes | 用于文件缓存的内存量 |
| Memory | `mem_shared` | Shared Memory | bytes | 多个进程间共享的内存量 |
| Memory | `mem_buffered` | Buffered Memory | bytes | 用于块设备I/O缓冲的内存量 |
...
| Disk | `disk_free` | Disk Free | bytes | 磁盘剩余可用空间 |
| Disk | `disk_inodes_used_percent` | Disk Inodes Usage | % | 文件系统inode已使用百分比 |
| Disk IO | `diskio_reads_total` | Disk Reads | counts | 磁盘累计读操作次数 |
| Disk IO | `diskio_writes_total` | Disk Writes | counts | 磁盘累计写操作次数 |
| Disk IO | `diskio_read_bytes_total` | Disk Read Bytes | bytes | 磁盘累计读取字节数 |
| Disk IO | `diskio_write_bytes_total` | Disk Write Bytes | bytes | 磁盘累计写入字节数 |
...
| Network | `net_packets_recv_rate` | Network Receive Packet Rate | cps | 网卡每秒接收的数据包数量 |
| Network | `net_packets_sent_rate` | Network Transmit Packet Rate | cps | 网卡每秒发送的数据包数量 |
| Network | `net_bytes_recv_rate` | Network Receive Byte Rate | byteps | 网卡每秒接收字节数 |
| Network | `net_bytes_sent_rate` | Network Transmit Byte Rate | byteps | 网卡每秒发送字节数 |
| Network | `net_err_in_rate` | Network Receive Error Rate | cps | 每秒接收的错误包数量 |
| Network | `net_err_out_rate` | Network Transmit Error Rate | cps | 每秒发送的错误包数量 |
| Network | `net_drop_in_rate` | Network Receive Drop Rate | cps | 每秒丢弃的入向数据包数量 |
| Network | `net_drop_out_rate` | Network Transmit Drop Rate | cps | 每秒丢弃的出向数据包数量 |
| Network | `net_packets_recv_total` | Network RX Packets | counts | 各网卡累计接收的数据包数 |
| Network | `net_packets_sent_total` | Network TX Packets | counts | 各网卡累计发送的数据包数 |
| Network | `net_drop_in_total` | Network RX Drops | counts | 各网卡累计丢弃的入向包数 |
| Network | `net_drop_out_total` | Network TX Drops | counts | 各网卡累计丢弃的出向包数 |
| Process | `processes_running` | Running Processes | counts | 当前处于运行态的进程数量 |
| Process | `processes_blocked` | Blocked Processes | counts | 因I/O或资源争用而阻塞的进程数量 |
| Process | `processes_zombies` | Zombie Processes | counts | 已退出但未被回收的僵尸进程数量 |
| Process | `processes_sleeping` | Sleeping Processes | counts | 当前处于睡眠或等待态的进程数量 |
...
### Haproxy（33 指标）
采集引擎：Telegraf、对象类别：Middleware  
_Collects HAProxy frontend/backend session, traffic, request, error, health-check and latency metrics to monitor load-balancer availability and performance._

> 证据来源：`server/apps/monitor/support-files/plugins/Telegraf/middleware/haproxy/metrics.json`（33 项，feat(monitor): add HAProxy monitoring support）　|　V1.0 基线中无此插件，本次新增

| 指标分组 | 指标名 | 显示名 | 单位 | 中文含义 |
|---|---|---|---|---|
| Session | `haproxy_scur` | Current Sessions | counts | 当前活跃的会话数量 |
| Session | `haproxy_smax` | Max Sessions | counts | 历史最大并发会话数 |
| Session | `haproxy_slim` | Session Limit | counts | 配置的最大会话数上限 |
| Session | `haproxy_stot` | Total Sessions | counts | 累计处理的会话总数 |
| Session | `haproxy_rate` | Session Rate | cps | 每秒新建会话速率 |
| Session | `haproxy_rate_max` | Max Session Rate | cps | 历史最大每秒新建会话速率 |
| Queue | `haproxy_qcur` | Current Queued Requests | counts | 当前排队等待的请求数 |
| Queue | `haproxy_qmax` | Max Queued Requests | counts | 历史最大排队请求数 |
| Traffic | `haproxy_bin` | Bytes In | bytes | 累计入向字节数 |
| Traffic | `haproxy_bout` | Bytes Out | bytes | 累计出向字节数 |
| HTTP | `haproxy_req_rate` | HTTP Request Rate | cps | 每秒 HTTP 请求速率 |
| HTTP | `haproxy_req_rate_max` | Max HTTP Request Rate | cps | 历史最大每秒 HTTP 请求速率 |
| HTTP | `haproxy_req_tot` | Total HTTP Requests | counts | 累计 HTTP 请求总数 |
| HTTP Response | `haproxy_hrsp_2xx` | 2xx Responses | counts | 累计 2xx 成功响应数 |
| HTTP Response | `haproxy_hrsp_3xx` | 3xx Responses | counts | 累计 3xx 重定向响应数 |
| HTTP Response | `haproxy_hrsp_4xx` | 4xx Responses | counts | 累计 4xx 客户端错误响应数 |
| HTTP Response | `haproxy_hrsp_5xx` | 5xx Responses | counts | 累计 5xx 服务端错误响应数 |
| Error | `haproxy_ereq` | Request Errors | counts | 累计请求错误数（解析/协议错误） |
| Error | `haproxy_econ` | Connection Errors | counts | 累计连接错误数（后端建连失败） |
| Error | `haproxy_eresp` | Response Errors | counts | 累计响应错误数（响应解析失败） |
| Denied | `haproxy_dreq` | Denied Requests | counts | 被 ACL 或限速拒绝的请求数 |
| Denied | `haproxy_dresp` | Denied Responses | counts | 被 ACL 拒绝的响应数 |
| Retry | `haproxy_wretr` | Connection Retries | counts | 后端连接重试次数 |
| Retry | `haproxy_wredis` | Request Redispatches | counts | 因重试失败而重分发到其他后端的请求数 |
| Backend | `haproxy_act` | Active Servers | counts | 当前处于活跃状态的后端服务器数量 |
| Backend | `haproxy_bck` | Backup Servers | counts | 当前处于备用状态的后端服务器数量 |
| Health | `haproxy_chkfail` | Failed Health Checks | counts | 累计健康检查失败次数 |
| Health | `haproxy_chkdown` | Down Transitions | counts | 后端服务器从 up 转 down 的次数 |
| Health | `haproxy_downtime` | Total Downtime | s | 后端服务器累计下线时长 |
| Latency | `haproxy_qtime` | Average Queue Time | ms | 请求在队列中等待的平均时间 |
| Latency | `haproxy_ctime` | Average Connect Time | ms | 建立后端连接的平均耗时 |
| Latency | `haproxy_rtime` | Average Response Time | ms | 后端响应的平均耗时 |
| Latency | `haproxy_ttime` | Average Total Time | ms | 请求从进入队列到响应完成的平均总耗时 |

### Firewall Flow NetFlow · Firewall（21 指标）
采集引擎：Telegraf、对象类别：Network Device  
_Collect firewall traffic from NetFlow and convert it into normalized device-level flow metrics, including total/directional rates, interface Top-N, protocol distribution, port distribution, and session-level Top-N._

| 指标分组 | 指标名 | 显示名 | 单位 | 中文含义 |
|---|---|---|---|---|
| Device Flow | `device_flow_bytes_rate` | Device Flow Bytes Rate | byteps | 设备总流量速率（字节/秒） |
| Device Flow | `device_flow_packets_rate` | Device Flow Packets Rate | cps | 设备总包速率 |
| Device Flow | `device_flow_in_bytes_rate` | Device Flow In Bytes Rate | byteps | 设备入向流量速率 |
| Device Flow | `device_flow_out_bytes_rate` | Device Flow Out Bytes Rate | byteps | 设备出向流量速率 |
| Device Flow | `device_flow_in_packets_rate` | Device Flow In Packets Rate | cps | 设备入向包速率 |
| Device Flow | `device_flow_out_packets_rate` | Device Flow Out Packets Rate | cps | 设备出向包速率 |
| Device Flow | `device_flow_avg_packet_size` | Device Flow Avg Packet Size | bytes | 设备平均包大小 |
| Device Flow | `device_flow_effective_sampling_rate` | Device Flow Effective Sampling Rate | none | 有效采样率 |
| Interface | `device_flow_interface_bytes_rate` | Interface Bytes Rate | byteps | 各接口流量速率（按接口维度） |
| Interface | `device_flow_interface_packets_rate` | Interface Packets Rate | cps | 各接口包速率 |
| Interface | `device_flow_top_interfaces_by_bytes` | Top Interfaces By Bytes | byteps | 流量 Top-N 接口 |
| Protocol | `device_flow_protocol_bytes_rate` | Protocol Bytes Rate | byteps | 各协议流量速率 |
| Protocol | `device_flow_protocol_packets_rate` | Protocol Packets Rate | cps | 各协议包速率 |
| Port | `device_flow_dst_port_bytes_rate` | Dst Port Bytes Rate | byteps | 各目标端口流量速率 |
| Port | `device_flow_dst_port_packets_rate` | Dst Port Packets Rate | cps | 各目标端口包速率 |
| Port | `device_flow_src_port_bytes_rate` | Src Port Bytes Rate | byteps | 各源端口流量速率 |
| Top-N | `device_flow_top_src_bytes_rate` | Top Src Bytes Rate | byteps | Top-N 源 IP 流量速率 |
| Top-N | `device_flow_top_dst_bytes_rate` | Top Dst Bytes Rate | byteps | Top-N 目标 IP 流量速率 |
| Top-N | `device_flow_top_src_packets_rate` | Top Src Packets Rate | cps | Top-N 源 IP 包速率 |
| Top-N | `device_flow_top_dst_packets_rate` | Top Dst Packets Rate | cps | Top-N 目标 IP 包速率 |
| Top-N | `device_flow_top_conversation_bytes_rate` | Top Conversation Bytes Rate | byteps | Top-N 会话对话流量速率 |

> 证据来源：`server/apps/monitor/support-files/plugins/Telegraf/netflow/firewall/metrics.json`（21 项 device_flow_* 指标）　|　同步基线：0fbb99c25　|　【已实现】

### Loadbalance Flow NetFlow · Loadbalance（21 指标）
采集引擎：Telegraf、对象类别：Network Device  
_Collect load balancer traffic from NetFlow and convert it into normalized device-level flow metrics. Same metric set as Firewall Flow NetFlow._

> 与 Firewall Flow NetFlow 指标集一致（21 项 `device_flow_*`），对象为 Loadbalance 负载均衡。证据：`server/apps/monitor/support-files/plugins/Telegraf/netflow/loadbalance/metrics.json`　|　同步基线：0fbb99c25　|　【已实现】

### Router Flow NetFlow · Router（21 指标）
采集引擎：Telegraf、对象类别：Network Device  
_Collect router traffic from NetFlow and convert it into normalized device-level flow metrics. Same metric set as Firewall Flow NetFlow._

> 与 Firewall Flow NetFlow 指标集一致（21 项 `device_flow_*`），对象为 Router 路由器。证据：`server/apps/monitor/support-files/plugins/Telegraf/netflow/router/metrics.json`　|　同步基线：0fbb99c25　|　【已实现】

### Switch Flow NetFlow · Switch（21 指标）
采集引擎：Telegraf、对象类别：Network Device  
_Collect switch traffic from NetFlow and convert it into normalized device-level flow metrics. Same metric set as Firewall Flow NetFlow._

> 与 Firewall Flow NetFlow 指标集一致（21 项 `device_flow_*`），对象为 Switch 交换机。证据：`server/apps/monitor/support-files/plugins/Telegraf/netflow/switch/metrics.json`　|　同步基线：0fbb99c25　|　【已实现】
...
### Windows WMI · Host（28 指标）
采集引擎：Telegraf (HTTP)、对象类别：OS  
_Collects Windows host metrics via WMI (Windows Management Instrumentation) through the HTTP collect type, covering CPU, memory, disk, disk I/O, network and process metrics. No agent required on the target host._

> 证据来源：`server/apps/monitor/support-files/plugins/Telegraf/http/windows_wmi/metrics.json`（28 项，feat: add windows wmi monitor plugin template）　|　V1.0 基线中无此插件，本次新增；object_name=Host，instance_type=os，collect_type=http

| 指标分组 | 指标名 | 显示名 | 单位 | 中文含义 |
|---|---|---|---|---|
| CPU | `cpu_usage_total` | CPU Usage | percent | 主机CPU整体使用率，反映非空闲CPU时间占比 |
| Memory | `mem_used_percent` | Memory Usage | percent | 内存已使用百分比，反映整体内存压力 |
| Memory | `mem_total` | Memory Total | bytes | 系统物理内存总量 |
| Memory | `mem_available` | Memory Available | bytes | 可供应用使用的可用内存量 |
| Disk | `disk_used_percent` | Disk Usage | percent | 磁盘空间已使用百分比 |
| Disk | `disk_total` | Disk Total | bytes | 磁盘或文件系统总容量 |
| Disk | `disk_free` | Disk Free | bytes | 磁盘剩余可用空间 |
| Disk IO | `diskio_io_util` | Disk IO Utilization | percent | 磁盘I/O繁忙时间占比 |
| Disk IO | `diskio_reads_rate` | Disk Read Rate | cps | 每秒磁盘读操作次数 |
| Disk IO | `diskio_writes_rate` | Disk Write Rate | cps | 每秒磁盘写操作次数 |
| Disk IO | `diskio_read_bytes_rate` | Disk Read Byte Rate | byteps | 每秒磁盘读取字节数 |
| Disk IO | `diskio_write_bytes_rate` | Disk Write Byte Rate | byteps | 每秒磁盘写入字节数 |
| Network | `net_packets_recv_rate` | Network Receive Packet Rate | cps | 网卡每秒接收的数据包数量 |
| Network | `net_packets_sent_rate` | Network Transmit Packet Rate | cps | 网卡每秒发送的数据包数量 |
| Network | `net_bytes_recv_rate` | Network Receive Byte Rate | byteps | 网卡每秒接收字节数 |
| Network | `net_bytes_sent_rate` | Network Transmit Byte Rate | byteps | 网卡每秒发送字节数 |
...
| Process | `processes_sleeping` | Sleeping Processes | counts | 当前处于睡眠或等待态的进程数量 |
| System | `system_uptime` | System Uptime | s | 系统持续运行时长 |
| System | `system_load1` | System Load 1m | — | 系统近1分钟平均负载 |
| System | `system_load5` | System Load 5m | — | 系统近5分钟平均负载 |
| System | `system_load15` | System Load 15m | — | 系统近15分钟平均负载 |
...
### Firewall Flow sFlow · Firewall（21 指标）
采集引擎：Telegraf、对象类别：Network Device  
_Collect firewall traffic from sFlow and convert it into normalized device-level flow metrics. Same device_flow_* metric set as NetFlow counterpart._

> 与 Firewall Flow NetFlow 指标集一致（21 项 `device_flow_*`），协议为 sFlow，对象为 Firewall 防火墙。证据：`server/apps/monitor/support-files/plugins/Telegraf/sflow/firewall/metrics.json`　|　同步基线：0fbb99c25　|　【已实现】

### Loadbalance Flow sFlow · Loadbalance（21 指标）
采集引擎：Telegraf、对象类别：Network Device  
_Collect load balancer traffic from sFlow and convert it into normalized device-level flow metrics._

> 与 Firewall Flow NetFlow 指标集一致（21 项 `device_flow_*`），协议为 sFlow，对象为 Loadbalance 负载均衡。证据：`server/apps/monitor/support-files/plugins/Telegraf/sflow/loadbalance/metrics.json`　|　同步基线：0fbb99c25　|　【已实现】

### Router Flow sFlow · Router（21 指标）
采集引擎：Telegraf、对象类别：Network Device  
_Collect router traffic from sFlow and convert it into normalized device-level flow metrics._

> 与 Firewall Flow NetFlow 指标集一致（21 项 `device_flow_*`），协议为 sFlow，对象为 Router 路由器。证据：`server/apps/monitor/support-files/plugins/Telegraf/sflow/router/metrics.json`　|　同步基线：0fbb99c25　|　【已实现】

### Switch Flow sFlow · Switch（21 指标）
采集引擎：Telegraf、对象类别：Network Device  
_Collect switch traffic from sFlow and convert it into normalized device-level flow metrics._

> 与 Firewall Flow NetFlow 指标集一致（21 项 `device_flow_*`），协议为 sFlow，对象为 Switch 交换机。证据：`server/apps/monitor/support-files/plugins/Telegraf/sflow/switch/metrics.json`　|　同步基线：0fbb99c25　|　【已实现】
...
| 指标分组 | 指标名 | 显示名 | 单位 | 中文含义 |
|---|---|---|---|---|
| HTTP | `http_response_result_code` | Probe Result | Enum（0 成功/1 内容不匹配/2 响应体读取失败/3 连接失败/4 超时/5 DNS 错误/6 状态码不匹配） | 探测结果枚举；带 `agent_id` 维度，按探测节点区分，置为首位 display_field |
...
> 证据来源：`server/apps/monitor/support-files/plugins/Telegraf/web/web/metrics.json`（metrics[0].name=http_response_result_code，display_fields sort_order=0）　|　同步基线：0fbb99c25　|　【已实现】

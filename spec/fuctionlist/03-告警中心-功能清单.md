...
**编制依据：** 告警中心 PRD v1.2（2026-05-28）与 `server/apps/alerts`、`web/src/app/alarm` 源代码核对；V1.1 增量基于 2026-06-18 代码核对（base.py:552-568、aggregation_processor.py:136,419、instant_dispatcher.py:273,310,315）；V1.2 增量基于 2026-06-22 enrichment 引擎落地核对（models/enrichment.py、enrichment/engine.py、views/enrichment.py、constants/init_data.py:248-270）；V1.3 增量基于 2026-06-22 即时告警旁路（instant_dispatcher.py:1-319、migrations/0017）+ 告警升级链（escalation_service.py、models/alert_operator.py:105-127）+ 丰富匹配器正则操作符（enrichment/matcher.py:19-24）核对
...
> 本附录列出 告警中心 模块的关键枚举与对象取值，取自源码常量定义。共 28 类、96 项取值（V1.3 新增：告警策略类型补 instant、丰富匹配操作符补 re、告警升级链模式 2 项）。
...
> 证据来源：`models/enrichment.py:ON_MULTIPLE_CHOICES`　|　同步基线：2026-06-22　|　【已实现/已存在】
...
> 证据来源：`enrichment/matcher.py:_cmp`，`enrichment/matcher.py:19-24`（正则操作符 `re` 新增于 2026-06-22）　|　同步基线：2026-06-22　|　【已实现/已存在】
...
> 证据来源：`service/escalation_service.py:17,59-67`（VALID_MODES + compute_roster）　|　同步基线：2026-06-22　|　【已实现/已存在】

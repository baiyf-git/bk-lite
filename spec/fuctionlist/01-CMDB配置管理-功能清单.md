...
| 功能项 | 功能说明 | 规格 / 约束 | 状态 |
|---|---|---|---|
| 全局全文检索 | 跨模型全文检索资产 | 可配置是否区分大小写；附件/图片（file 型）字段按文件名词干（去路径、去扩展名）可被检索命中，URL/大小/ID 等元数据不进索引（`display_field/handler.py:206-249`）；密码（pwd）字段为密文，刻意完全排除出全文检索，不生成 _display、不可被搜索命中（`display_field/constants.py:17`、`display_field/cache.py:431`） | GA |
...
| 变更场景归类 | 每条记录归入一种变更场景 | 6 类：设备流转、关系变更、普通属性变更、自动采集/自动化变更、模型管理变更、自定义上报变更（`models/change_record.py:22-27` 枚举常量） | GA |
| 场景判定与修正 | 固定入口场景由系统判定且不可改；通用编辑入口给默认场景，可在少量相邻场景间轻量修正 | 首期不开放任意切换 | GA |
| 来源分类 | 记录变更来源（人工、自动采集、系统、导入、同步），与变更场景分开 | — | GA |
| 管理类变更镜像至平台操作日志 | 模型管理变更、自动采集/自动化变更、自定义上报变更、关系变更（_MIRROR_SCENARIOS）在写入 CMDB 自身变更记录后，经 SystemMgmt NATS RPC（`save_operation_log`）同步镜像一份至平台级操作日志，供统一审计；镜像为尽力而为，失败仅 warning 记录，不阻断 CMDB 变更记录写入 | 4 类管理场景触发镜像；普通属性变更（`ORDINARY_ATTRIBUTE_CHANGE`）不镜像；来源：`utils/change_record.py:18-49` | GA |

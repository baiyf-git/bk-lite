...
| OperatorLog | `models/operator_log.py`（operator_log.py:10） | 操作审计；写入统一经 `utils/operator_log.py` 的 `record_operator_log` / `record_operator_logs_bulk` 入口，写本地表的同时经 `SystemMgmt().save_operation_log` 镜像进平台操作日志（`app="alarm"`，`LogAction→create/update/delete/execute` 映射，`detail` 携带 `operator_object/source=operator_log`）；镜像失败仅 `logger.warning`，绝不阻断源写入【已实现/已存在，`utils/operator_log.py:25-63`】 |
...
**团队上下文读取**【已实现/已存在】：`permission_scope.py` 与 `views/alert_source.py` 中的团队归属裁剪统一经 `apps.core.utils.team_utils.get_current_team(request)` 读取，优先取 `APISecretMiddleware` 注入的 `request._api_current_team`，其次取浏览器 cookie `current_team`，均不存在时返回 `None`（`team_utils.py:23-41`）。此前两处均直接读 `request.COOKIES.get("current_team")`，导致 API Key 调用路径的组织上下文缺失；现统一后，API Key 与浏览器两种调用路径的 `permission_scope` 团队范围裁剪及 `alert_source` 的 `_get_valid_current_team` 校验行为保持一致【已实现/已存在，`utils/permission_scope.py:12`；`views/alert_source.py:51`；`apps/core/utils/team_utils.py:23-41`】。
...
- 操作日志镜像【已实现/已存在】：`utils/operator_log.py` 的 `_mirror()` 函数经 `apps.rpc.system_mgmt.SystemMgmt().save_operation_log`（`system_mgmt.py:178-193`）将本模块 `OperatorLog` 数据镜像写入平台统一操作日志（`app="alarm"`）。调用点覆盖 5 个 view（`assignment_shield`、`incident`、`incident_update`、`strategy`、`system_setting`）、2 个 service（`alter_operator.py:591`、`incident_operator.py:209`）、2 个 common（`assignment.py:190`、`auto_close.py:261`）。镜像失败仅 `logger.warning`，不阻断源写入，构成 alerts → system_mgmt 的操作日志契约【已实现/已存在，`utils/operator_log.py:25-63`；`apps/rpc/system_mgmt.py:178-193`】。
